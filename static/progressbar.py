...
# for bar_cls in (ChargingBar, FillingSquaresBar, FillingCirclesBar):
...
def charging(message):
    for bar_cls in (ChargingBar,):
        suffix = '%(index)d/%(max)d [%(elapsed)d / %(eta)d / %(eta_td)s]'
        bar = bar_cls(message, suffix=suffix)
        for i in bar.iter(range(200)):
            sleep()


def fillingsquares(message):
    for bar_cls in (FillingSquaresBar,):
        suffix = '%(index)d/%(max)d [%(elapsed)d / %(eta)d / %(eta_td)s]'
        bar = bar_cls(message, suffix=suffix)
        for i in bar.iter(range(200)):
            sleep()
...
def spiner(message):
    for spin in (Spinner,):
        for i in spin(message + ' ').iter(range(100)):
            sleep()
